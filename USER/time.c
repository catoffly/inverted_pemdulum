...
u8  TIM5CH1_CAPTURE_STA=0,TIM2CH1_CAPTURE_STA=0,TIM3CH1_CAPTURE_STA=0;	//输入捕获状态		    				
u16	TIM5CH1_CAPTURE_VAL,TIM2CH1_CAPTURE_VAL,TIM3CH1_CAPTURE_VAL;	//输入捕获值
...
{	 		
	...
	//定时器TIM3初始化
	TIM_TimeBaseStructure.TIM_Period = 49; //设置在下一个更新事件装入活动的自动重装载寄存器周期的值	
...
/************************************************************************************************
	* 函数名：TIM8_Configuration
	* 函数作用：TIM8外部脉冲计数模式初始化
	* 参数：空
	* 库版本  ：ST3.5.0
*************************************************************************************************/
...
// 	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable; //比较输出使能
//	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_Low; //输出极性:TIM输出比较极性高
//	TIM_OC3Init(TIM4, &TIM_OCInitStructure);  //根据T指定的参数初始化外设TIM5 OC3
//	
//	TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM2; //选择定时器模式:TIM脉冲宽度调制模式2
// 	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable; //比较输出使能
//	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_Low; //输出极性:TIM输出比较极性高
//	TIM_OC4Init(TIM4, &TIM_OCInitStructure);  //根据T指定的参数初始化外设TIM5 OC4
	...
	TIM_OC1PreloadConfig(TIM4, TIM_OCPreload_Enable);  //使能TIM3在CCR3上的预装载寄存器
...
// 	TIM_SetCompare4(TIM4,0);//防止开机电机转动
...
void TIM4_IRQHandler(void)
{
	...
	{
		n=0;
		if(!(a_direction^v_direction))//相同加速，相反减速。
		{step++;}
		else
		{step--;}
		meibu=1;
...
//		printf("字符串输出 d4=%s\r\n",buff);
	...
	TIM_ClearITPendingBit(TIM4, TIM_IT_Update|TIM_IT_CC1); //清除中断标志位
}
